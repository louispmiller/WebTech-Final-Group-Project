...
Made sure cities and weather_data tables exist and seeded one city.
Ran php -S localhost:8000 from the project root.
curl "http://localhost:8000/api/weather?city_id=1" — first call hits Open-Meteo live and stores it, calls within 10 minutes after that just return the cached row.
curl -X POST -H "Content-Type: application/json" -d '{"city_id":1}' http://localhost:8000/api/weather/current to force a refresh.
